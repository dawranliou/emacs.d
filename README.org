...
up =gc-cons-threshold= (and perhaps =gc-cons-percentage=) temporarily in the
early-init.el.  However, it is important to reset it eventually. Not doing so
will cause garbage collection freezes during long-term interactive
